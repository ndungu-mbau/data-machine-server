...
// const projectQuestionnaire = questionnaireId => async (attrs, { datastore }) =>
...
const teamProjects = teamId => async (filter, { datastore }) => {
  ...
  return entities.shift().map(entry => Object.assign(
    {},
    entry,
    {
  ...
  team: {
    users: async ({ id }, args, { db, ObjectId }) => {
      ...
      const users = await db
        .collection('user')
        .find({
          _id: { $in: relations.map(relation => ObjectId(relation.user)) },
          destroyed: false,
        })
        .toArray();
    ...
    // const relations = await db.collection('dashboard_teams')
    // .find({ team: id.toString() }).toArray();

    // const dashboards = await db.collection('dashboards')
    // .find({ _id: { $in: relations.map(relation => ObjectId(relation.dashboard)) },
    // destroyed: false }).toArray();
    ...
    dashboards: async () => [
      {
        ...
        url:
          'https://metabase.braiven.io/public/dashboard/fcc20dcb-544c-44c8-9dd5-ddc0318e8be0',
      },
    ],
    projects: async ({ id }, args, { db, ObjectId }) => {
      ...
      const projects = await db
        .collection('project')
        .find({
          _id: { $in: relations.map(relation => ObjectId(relation.project)) },
          destroyed: false,
        })
        .toArray();
