...
MongoClient.connect(
  ...
  (err, client) => {
    ...
    jobs.forEach(({ name, schedule, work, options, emediate }) => {
...
app.post("/saasAuth/activateInvitation/:id", async (req, res) => {
  ...
  if (!user) {
    ...
    db.collection("invitation").updateOne(
      {
        user: userEmail,
        client
      },
...
hemera.add(
  ...
  args =>
    new Promise(async (resolve, reject) => {
      const { password, email, contact, firstName, orgName } = args.data;
...
app.get("/submision/breakDown/:days", auth, async (req, res) => {
  ...
  Object.keys(weeks).map(async weekKey =>
    Object.keys(weeks[weekKey].daysInWeek).map(async day => {
      promises.push(
...
app.get("/submisions/:questionnaireId", async (req, res) => {
  // try {
  const { showComputed = 1, showCompounded = 1, filters = [] } = req.query;
  ...
  return res.send(
    Object.assign(
      {},
      // eslint-disable-next-line eqeqeq
      parseInt(showCompounded, 10) ? { compounded } : { compounded: [] },
      // eslint-disable-next-line eqeqeq
      parseInt(showComputed, 10) ? { computed } : { computed: [] },
